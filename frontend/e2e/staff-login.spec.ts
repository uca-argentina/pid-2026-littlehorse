...
const productsPath = `${staffAreaPath}/products`;
...
test.describe('Staff login', () => {
  ...
  // Straight to the menu: an administrator signs in to manage it, and a home
  // screen in between was one more tap for nothing.
  test('opens the products for the seeded administrator', async ({ page }) => {
    ...
    // The account comes from the token, and the header shows whose it is and
    // what it can do.
    await expect(
      page.getByRole('button', { name: new RegExp(`${seededAdminUsername} · administrador`, 'i') }),
    ).toBeVisible();
  });
