...
/**
 * How one of the two things this screen can do ended up. Tracked apart on
 * purpose, same reasoning as EditStaffUserStore: a correction that gets
 * refused must not make the deactivation next to it look like it failed too.
 */
...
export class EditProductStore {
  ...
  readonly isBusy = computed(() => this.details() === 'sending' || this.access() === 'sending');
  ...
  /**
   * The two actions differ only in which request they send. Writing them out
   * twice would be two places to forget the in-flight guard.
   */
  private run(
